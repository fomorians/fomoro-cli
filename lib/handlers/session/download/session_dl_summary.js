...
function downloadArtifact(model, sha) {
  ...
  let filename = `summary_${sha_}.tar.gz`;

  console.log(`This will download summary artifacts for "${model.name}" at ${sha_}.`);
  console.log();
  ...
  read({ prompt: 'Is this okay? (y/n)' }, (err, yn) => {
    ...
    if (yn.toLowerCase() === 'y') {
      ...
      var req = utils.postJSON(config.host + config.session_download_summary_endpoint, { model: model.name, sha: sha }, function(err, res, body) {
        if (err) {
          console.error(exceptions.getUserMessage(err));
          process.exit(1);
