...
function downloadArtifact(model, sha) {
  ...
  let filename = `model_${sha_}.tar.gz`;

  console.log(`This will download model artifacts for "${model.name}" at ${sha_}.`);
  console.log();
  ...
  read({ prompt: 'Is this okay? (y/n)' }, (err, yn) => {
    ...
    if (yn.toLowerCase() === 'y') {
      ...
      var req = utils.postJSON(config.host + config.session_download_model_endpoint, { model: model.name, sha: sha }, function(err, res, body) {
        if (err) {
          console.error(exceptions.getUserMessage(err));
          process.exit(1);
