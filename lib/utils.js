...
var shaTest = /^[a-f0-9]{40}$/;
var idRegExp = /^(?:([a-zA-Z0-9\_\-]+)\/)?(?:([a-zA-Z0-9\_\-]+)\#)?([a-f0-9]{40})$/;
...
exports.getModelName = function(id) {
  if (id) {
    var matches = id.match(idRegExp);
    if (matches && matches[2]) {
      return matches[2];
    }
  }

  let model = exports.readConfig();
  return model.name;
};
...
exports.getSha = function(id) {
  ...
  if (id) {
    var matches = id.match(idRegExp);
    if (matches) {
      return Promise.resolve(matches[3]);
    }
  }
...
exports.readConfig = function() {
  try {
    ...
    console.error('Could not open fomoro.toml config:', err.messsage);
    process.exit(1);
  }
};
